...
try {
  ...
  console.log('PASS',results.filter(result=>result.type==='page').length,'ownership/ancillary page observations and',links.size,'distinct internal page/fragment links');
} finally {
  await fs.writeFile('../../evidence/growth-review/internal-link-review.json',JSON.stringify(results,null,2));
  await browser.close();
}
