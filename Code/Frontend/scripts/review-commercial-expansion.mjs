...
const route = '/new-jersey/commercial-solar';
const output = process.env.CRR_REVIEW_OUT || '../../evidence/commercial-expansion-review.json';
...
try {
  for (const [size, viewport] of [['desktop', { width: 1440, height: 1000 }], ['mobile', { width: 390, height: 844 }]]) {
    ...
    await expect(page.getByLabel('Project state or district')).toHaveValue('NJ');
    await expect(page.locator('link[rel="canonical"]')).toHaveAttribute('href', `https://ratereliefca.com${route}`);
    results.push({ route, size, status: 200, market: 'NJ', canonical: true, commercialForm: true, passed: true });
    if (size === 'mobile') {
      const file = path.resolve(path.dirname(output), 'new-jersey-commercial-solar-mobile.png');
