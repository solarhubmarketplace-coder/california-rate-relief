...
const route = '/solar-problems/solar-homeowners-insurance';
const output = process.env.CRR_REVIEW_OUT || '../../evidence/solar-insurance-owner-review.json';
...
try {
  for (const [size, viewport] of [['desktop', { width: 1440, height: 1000 }], ['mobile', { width: 390, height: 844 }]]) {
    ...
    results.push({ route, size, status: 200, market: 'CA', canonical: true, form: true, sources: true, passed: true });
    if (size === 'mobile') {
      const file = path.resolve(path.dirname(output), 'solar-homeowners-insurance-mobile.png');
