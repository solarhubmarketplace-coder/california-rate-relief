...
const route = '/blog/is-my-roof-good-for-solar-california';
const output = process.env.CRR_REVIEW_OUT || '../../evidence/roof-suitability-refresh-review.json';
...
try {
  for (const [size, viewport] of [['desktop', { width: 1440, height: 1000 }], ['mobile', { width: 390, height: 844 }]]) {
    ...
    await expect(page.locator('#quote-checklist')).toBeVisible();
    ...
    results.push({ route, size, status: 200, market: 'CA', canonical: true, form: true, checklist: true, sources: true, passed: true });
    if (size === 'mobile') {
      const file = path.resolve(path.dirname(output), 'is-my-roof-good-for-solar-california-mobile.png');
