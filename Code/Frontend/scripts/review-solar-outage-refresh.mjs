...
const route = '/blog/do-solar-panels-work-during-power-outage-california';
const output = process.env.CRR_REVIEW_OUT || '../../evidence/solar-outage-refresh-review.json';
...
try {
  for (const [size, viewport] of [['desktop', { width: 1440, height: 1000 }], ['mobile', { width: 390, height: 844 }]]) {
    ...
    await expect(page.getByText('Backup proposal checklist')).toBeVisible();
    ...
    results.push({ route, size, status: 200, market: 'CA', canonical: true, form: true, backupChecklist: true, sources: true, passed: true });
    if (size === 'mobile') {
      const file = path.resolve(path.dirname(output), 'solar-outage-california-mobile.png');
