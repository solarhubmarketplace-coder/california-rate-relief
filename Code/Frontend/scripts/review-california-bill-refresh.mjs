...
const output = process.env.CRR_REVIEW_OUT || '../../evidence/california-bill-refresh-review.json';
...
try {
  for (const [size, viewport] of [['desktop', { width: 1440, height: 1000 }], ['mobile', { width: 390, height: 844 }]]) {
    for (const route of routes) {
      ...
      await expect(page.locator('#bill-comparison')).toBeVisible();
      ...
      results.push({ route, size, status: 200, market: 'CA', canonical: true, form: true, calculator: true, sources: true, passed: true });
      if (size === 'mobile') {
        const file = path.resolve(path.dirname(output), `${route.split('/').at(-1)}-mobile.png`);
  ...
  for (const route of routes) expect(sitemapText).toContain(route);
  results.push({ case: 'sitemap', status: 200, routes: routes.length, passed: true });
} finally {
